...
16.11.2021  Запустить на BSFrance (не используя пока Лору)
===============
...
===============
  Соединения:
    BNO080 (модуль GY-BNO08X):
      GND -> GND
      VCC -> 3.3v BSFrance LoRa32u4 (эта конкретная плата датчика не имеет преобразователя 5в->3.3в)
      SCL -> A5 BSFrance LoRa32u4 (возможно, настраивается библиотекой Wire)
      SDA -> A4 BSFrance LoRa32u4 (возможно, настраивается библиотекой Wire)
    Лента WS2812B - Для передатчика она не нужна! Но для случаев настройки след. инфа:
    13 светодиодов (задаётся в коде - в принципе любое, из-за среднего "нуля" напрашивается нечётное число):
    ...
    Control button -> "NO" контакты: 
      GND 
      D6 BSFrance LoRa32u4 (задаётся) NOTE!!! На обратной стороне BSFrance надо перерезать перемычку "DIO10->6 default closed"
  Функции кнопки:
